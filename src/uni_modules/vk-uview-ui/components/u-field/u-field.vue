<template>
  <view
    class="u-field"
    :class="{ 'u-border-top': borderTop, 'u-border-bottom': borderBottom }"
  >
    <view
      class="u-field-inner"
      :class="[
        type == 'textarea' ? 'u-textarea-inner' : '',
        'u-label-postion-' + labelPosition,
      ]"
    >
      <view
        class="u-label"
      ...
      >
        ...
        <text
          class="u-label-text"
          :class="[$slots.icon || icon ? 'u-label-left-gap' : '']"
          >{{ label }}</text
        >
      </view>
      <view class="fild-body">
        ...
        <u-icon
          v-if="clearable && valueCom != '' && focused"
          :size="clearSize"
...
export default {
  ...
  computed: {
    ...
    rightIconStyle() {
      const style = {}
      const arrowDirectionObj = {
        top: '-90deg',
        bottom: '90deg',
        left: '180deg',
        right: '0deg',
      }
      const deg = arrowDirectionObj[this.arrowDirection] || '0deg'
