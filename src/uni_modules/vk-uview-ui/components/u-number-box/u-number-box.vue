...
export default {
  ...
  watch: {
    ...
    inputVal(v1, v2) {
      ...
      const tmp = this.isNumber(v1)
      if (tmp && v1 >= this.min && v1 <= this.max) value = v1
      else value = v2
      // 判断是否只能输入大于等于0的整数
      if (
        this.positiveInteger && // 小于0，或者带有小数点，
        (v1 < 0 || String(v1).includes('.'))
      ) {
        value = v2
  ...
  methods: {
    ...
    calcPlus(num1, num2) {
      ...
      baseNum = 10 ** Math.max(baseNum1, baseNum2)
    ...
    computeVal(type) {
      ...
      if (this.stepStrictly) {
        const strictly = value % this.step
        if (strictly > 0) {
          value -= strictly
        }
