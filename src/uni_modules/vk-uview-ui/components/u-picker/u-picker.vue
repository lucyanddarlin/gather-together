...
export default {
  ...
  methods: {
    ...
    initTimeValue() {
      ...
      fdate = fdate && !fdate.includes('/') ? `2020/01/01 ${fdate}` : fdate
      let time = null
    ...
    setYears() {
      ...
      this.valueArr.splice(-1, 1, this.getIndex(this.years, this.year))
    },
    setMonths() {
    ...
    setMinutes() {
      ...
      this.valueArr.splice(-1, 1, this.getIndex(this.minutes, this.minute))
    },
    setSeconds() {
      this.seconds = this.generateArray(0, 59)
      this.valueArr.splice(-1, 1, this.getIndex(this.seconds, this.second))
    },
    setProvinces() {
      // 判断是否需要province参数
      ...
      if (this.areaCode.length > 0) {
        tmp = this.areaCode[0]
        useCode = true
      } else if (this.defaultRegion.length > 0) tmp = this.defaultRegion[0]
      else tmp = 0
      // 历遍省份数组匹配
      provinces.map((v, k) => {
    ...
    setCitys() {
      ...
      if (this.areaCode.length > 0) {
        tmp = this.areaCode[1]
        useCode = true
      } else if (this.defaultRegion.length > 0) tmp = this.defaultRegion[1]
      else tmp = 0
    ...
    setAreas() {
      ...
      if (this.areaCode.length > 0) {
        tmp = this.areaCode[2]
        useCode = true
      } else if (this.defaultRegion.length > 0) tmp = this.defaultRegion[2]
      else tmp = 0
    ...
    regionDiscern(addressText) {
      ...
      const firstTwoKey = addressText.slice(0, 2) //字符串开始的两个字符
      // 所在省的下标
      let provinceIndex = -1
      ...
      for (const [i, { name, code }] of citysArr.entries()) {
        // 除去最后一个 市 字
        const cityName = name.slice(0, Math.max(0, name.length - 1))
        // 用城市的名字去匹配 addressText 地址
        if (addressText.includes(cityName)) {
          city = { code, name }
          cityIndex = i
          break
        }
      ...
      for (const [i, { code, name }] of areasArr.entries()) {
        // 区名(县名)一般为3个字，如江干区，有可能会简写成 江干
        // 区名(县名)为2个字时，不会简写，如蓉县
        let reg = name
        if (name.length > 2)
          reg += `|${name.slice(0, Math.max(0, name.length - 1))}`
        // 查找地址中是否存在该区名
    ...
    addressDiscern(text) {
      ...
      const textArr = text.split(/[^\d()+-（）]+/g).filter((v) => v.length)
      if (textArr.length != 3)
        return {
          code: -1,
      ...
      for (const [k, v] of textArr.entries()) {
        if (/^1[,3-9]\d{9}$/.test(v)) {
          mobile = v
          continue
        }
        ...
        temp.length > v.length
          ? ((addressText = temp), (name = v))
          : ((addressText = v), (name = temp))
      }
      const positionRes = this.regionDiscern(addressText)
