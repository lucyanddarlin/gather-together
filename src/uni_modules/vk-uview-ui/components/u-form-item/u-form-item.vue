      ...
      <view class="u-form-item--right u-flex">
        <view class="u-form-item--right__content">
          <view
            ...
            :style="
              elLabelPosition == 'left' && elInputAlign == 'right'
                ? 'text-align:right;display: inline-block;line-height:initial;'
                : ''
            "
          >
            <slot />
...
export default {
  ...
  computed: {
    ...
    showError() {
      return (type) => {
        ...
        if (this.errorType.includes('none')) return false
        else if (this.errorType.includes(type)) return true
        else return false
      }
    },
    // label的宽度
    ...
    // label的样式
    elLabelStyle() {
      return Object.keys(this.labelStyle).length > 0
        ? this.labelStyle
  ...
  methods: {
    ...
    getFilteredRule(triggerType = '') {
      ...
      return rules.filter(
        (res) => res.trigger && res.trigger.includes(triggerType)
      )
    },
    ...
    validation(trigger, callback = () => {}) {
      ...
      validator.validate(
        ...
        (errors, fields) => {
          ...
          const field = errors ? errors[0].field : ''
          // 调用回调方法
