<template>
  <view :class="'interlayer ' + (c || '')" :style="s">
    <block v-for="(n, i) in nodes" :key="i">
      <!--图片-->
      <view
        v-if="n.name == 'img'"
...
export default {
  ...
  methods: {
    ...
    play(e) {
      const contexts = this.top.videoContexts
      if (contexts.length > 1 && this.top.autopause)
        for (let i = contexts.length; i--; )
    ...
    error(e) {
      ...
      if (source == 'video' || source == 'audio') {
        // 加载其他 source
        const index = this.ctrl[i] ? this.ctrl[i].i + 1 : 1
    ...
    use(item) {
      return (
        !item.c &&
        !inline[item.name] &&
        !(item.attrs.style || '').includes('display:inline')
      )
    },
  },
}
