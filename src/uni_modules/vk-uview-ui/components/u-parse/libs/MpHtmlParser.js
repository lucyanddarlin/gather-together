...
function MpHtmlParser(data, options = {}) {
  ...
  options.prot = (this.domain || '').includes('://')
    ? this.domain.split('://')[0]
    : 'http'
  ...
  this.decode = (val, amp) => {
    ...
    while (1) {
      ...
      if (val[i + 1] == '#') {
        en = Number.parseInt(
          (val[i + 2] == 'x' ? '0' : '') + val.substring(i + 2, j)
        )
        if (!isNaN(en))
          val =
            val.slice(0, Math.max(0, i)) +
            String.fromCharCode(en) +
            val.slice(j + 1)
      } else {
  ...
  this.siblings = () =>
    this.STACK.length > 0 ? this.parent().children : this.DOM
}
MpHtmlParser.prototype.parse = function () {
  if (emoji) this.data = emoji.parseEmoji(this.data)
  for (var c; (c = this.data[this.i]); this.i++) this.state(c)
  if (this.state == this.Text) this.setText()
  while (this.STACK.length > 0) this.popNode(this.STACK.pop())
  return this.DOM
}
// 设置属性
MpHtmlParser.prototype.setAttr = function () {
  ...
  if (cfg.boolAttrs[name]) this.attrs[name] = 'T'
    ...
    else if (name.slice(0, 5) != 'data-') this.attrs[name] = val
  }
  this.attrVal = ''
...
MpHtmlParser.prototype.setText = function () {
  ...
  if (back) {
    this.data =
      this.data.slice(0, Math.max(0, this.start)) +
      text +
      this.data.slice(this.i)
...
MpHtmlParser.prototype.setNode = function () {
  ...
  if (this.options.nodes.length > 0) node.type = 'node'
  this.attrs = {}
  if (!cfg.ignoreTags[node.name]) {
    ...
    switch (node.name) {
      ...
      case 'audio':
        ...
        if (node.name == 'video') {
          ...
          if (attrs.height) {
            styleObj.height =
              Number.parseFloat(attrs.height) +
    ...
    for (let i = 0, len = styles.length; i < len; i++) {
      ...
      if (value[0] == '-' || value.includes('safe')) style += `;${key}:${value}`
      else if (
        !styleObj[key] ||
    ...
    if (node.name == 'img') {
      ...
      if (styleObj.width) width = styleObj.width
        ...
        width = attrs.width.includes('%')
          ? attrs.width
          : `${Number.parseFloat(attrs.width)}px`
      if (width) {
        styleObj.width = width
        attrs.width = '100%'
        if (Number.parseInt(width) > windowWidth) {
          styleObj.height = ''
      ...
      if (styleObj.height) {
        ...
        attrs.height = `${Number.parseFloat(attrs.height)}px`
    }
    for (const key in styleObj) {
      ...
      if (value.includes('url')) {
        ...
        if (j++ != -1) {
          ...
          value = value.slice(0, Math.max(0, j)) + this.getUrl(value.slice(j))
        }
      }
      // 转换 rpx
    ...
    if (!close) {
      ...
      this.siblings().push(node)
  } else if (!close) this.remove(node)
...
MpHtmlParser.prototype.popNode = function (node) {
  ...
  if (node.name == 'table') {
    let padding = Number.parseFloat(attrs.cellpadding),
      spacing = Number.parseFloat(attrs.cellspacing),
      border = Number.parseFloat(attrs.border)
    if (node.c) {
    ...
    if (node.flag && node.c) {
      ...
      attrs.style = `${attrs.style || ''};${
        spacing ? `;grid-gap:${spacing}px` : ';border-left:0;border-top:0'
      }`
      let row = 1,
        col = 1,
        colNum,
      ...
      ;(function f(ns) {
        for (const n of ns) {
          if (n.name == 'tr') trs.push(n)
          else f(n.children || [])
        }
      })(node.children)
      for (const tr of trs) {
        for (let j = 0, td; (td = tr.children[j]); j++) {
          if (td.name == 'td' || td.name == 'th') {
            ...
            if (td.attrs.colspan) {
              cell.attrs.style += `;grid-column-start:${col};grid-column-end:${
                col + Number.parseInt(td.attrs.colspan)
              }`
              if (!td.attrs.rowspan)
                cell.attrs.style += `;grid-row-start:${row};grid-row-end:${
                  row + 1
                }`
            ...
            if (td.attrs.rowspan) {
              cell.attrs.style += `;grid-row-start:${row};grid-row-end:${
                row + Number.parseInt(td.attrs.rowspan)
              }`
              if (!td.attrs.colspan)
                cell.attrs.style += `;grid-column-start:${col};grid-column-end:${
                  col + 1
        ...
        (function f(ns) {
          for (var i = 0, n; (n = ns[i]); i++) {
            if (n.name == 'th' || n.name == 'td') {
              if (border)
                n.attrs.style = `border:${border}px solid gray;${
                  n.attrs.style || ''
                }`
              if (padding)
...
MpHtmlParser.prototype.EndTag = function (c) {
  if (blankChar[c] || c == '>' || c == '/') {
    const name = this.section().toLowerCase()
