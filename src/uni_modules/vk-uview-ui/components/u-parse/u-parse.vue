<template>
  <view>
    ...
    >
      ...
      <!--#endif-->
      <!--#ifndef H5 || MP-360-->
      <trees :nodes="nodes" :lazy-load="lazyLoad" :loading="loadingImg" />
      <!--#endif-->
    </view>
    <!--#endif-->
  </view>
...
const { windowWidth, platform } = uni.getSystemInfoSync()
// #endif
// #ifdef APP-PLUS-NVUE
  ...
  methods: {
    ...
    setContent(html, append) {
      ...
      if (append)
        this.$refs.web.evalJs(
          `var b=document.createElement('div');b.innerHTML='${html.replace(
            /'/g,
            "\\'"
          )}';document.getElementById('parser').appendChild(b)`
        )
      else {
        html = `<meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no"><style>html,body{width:100%;height:100%;overflow:hidden}body{margin:0}</style><base href="${
          this.domain
        }"><div id="parser"${
          ...
          this.showWithAnimation
            ...
            : ''
        }setTimeout(function(){e({action:"load",text:document.body.innerText,height:document.getElementById("parser").scrollHeight})},50);\u003C/script>`
      ...
      this.$refs.web.evalJs(
        `var t=document.getElementsByTagName("title");t.length&&e({action:"getTitle",title:t[0].innerText});for(var o,n=document.getElementsByTagName("style"),r=1;o=n[r++];)o.innerHTML=o.innerHTML.replace(/body/g,"#parser");for(var a,c=document.getElementsByTagName("img"),s=[],i=0==c.length,d=0,l=0,g=0;a=c[l];l++)parseInt(a.style.width||a.getAttribute("width"))>${windowWidth}&&(a.style.height="auto"),a.onload=function(){++d==c.length&&(i=!0)},a.onerror=function(){++d==c.length&&(i=!0),${
      ...
      // #ifdef H5 || MP-360
      if (!html) {
        if (this.rtf && !append) this.rtf.remove()
        return
      }
      const div = document.createElement('div')
      if (!append) {
        if (this.rtf) this.rtf.remove()
        this.rtf = div
      } else if (!this.rtf) this.rtf = div
      else this.rtf.append(div)
      div.innerHTML = this._handleHtml(html, append)
      for (
        var styles = this.rtf.querySelectorAll('style'), i = 0, style;
        (style = styles[i++]);
      ...
      const _ts = this
      // 获取标题
      const title = this.rtf.querySelectorAll('title')
      if (title.length > 0 && this.autosetTitle)
      ...
      const fill = (target) => {
        ...
        if (this.domain && src) {
          if (src[0] == '/') {
            if (src[1] == '/')
              target.src = `${
                this.domain.includes('://') ? this.domain.split('://')[0] : ''
              }:${src}`
      ...
      const imgs = this.rtf.querySelectorAll('img')
      for (let i = 0, j = 0, img; (img = imgs[i]); i++) {
        if (
          Number.parseInt(img.style.width || img.getAttribute('width')) >
          windowWidth
        )
        ...
        if (!img.hasAttribute('ignore') && img.parentElement.nodeName != 'A') {
          img.i = j++
          _ts.imgList.push(
            img.getAttribute('original-src') || img.src || img.dataset.src
          )
          img.addEventListener('click', function (e) {
            e.stopPropagation()
            let preview = _ts.preview
      ...
      // 链接处理
      const links = this.rtf.querySelectorAll('a')
      ...
      // 视频处理
      const videos = this.rtf.querySelectorAll('video')
      ...
      // 音频处理
      const audios = this.rtf.querySelectorAll('audio')
      ...
      // 表格处理
      if (this.autoscroll) {
        const tables = this.rtf.querySelectorAll('table')
      ...
      if (!append) this.document.append(this.rtf)
      this.$nextTick(() => {
        this.nodes = [1]
        this.$emit('load')
      })
      ...
      const parser = new Parser(html, this)
      // 缓存读取
      if (this.useCache) {
      ...
      if (nodes.length > 0 && nodes.title && this.autosetTitle)
        uni.setNavigationBarTitle({
          title: nodes.title,
        })
    ...
    _handleHtml(html, append) {
      ...
      if (html.includes('rpx'))
        html = html.replace(
          /[\d.]+\s*rpx/g,
          ($) => `${(Number.parseFloat($) * windowWidth) / 750}px`
        )
      return html
    },
    // #endif
