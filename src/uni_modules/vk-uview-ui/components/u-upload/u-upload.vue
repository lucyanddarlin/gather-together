<template>
  <view v-if="!disabled" class="u-upload">
    ...
    >
      ...
      ></u-line-progress>
      <view v-if="item.error" class="u-error-btn" @tap.stop="retry(index)"
        >点击重试</view
      >
      <image
        v-if="!item.isImage"
  ...
  methods: {
    ...
    selectFile() {
      ...
      chooseFile
        .then((res) => {
          const file = null
          const listOldLength = that.lists.length
    ...
    async uploadFile(index = 0) {
      ...
      if (this.beforeUpload && typeof this.beforeUpload === 'function') {
        ...
        const beforeResponse = this.beforeUpload.bind(
        ...
        success: async (res) => {
          if (res.confirm) {
            ...
            if (this.beforeRemove && typeof this.beforeRemove === 'function') {
              ...
              const beforeResponse = this.beforeRemove.bind(
                this.$u.$parent.call(this)
              )(index, this.lists)
              // 判断是否返回了promise
              if (
